...
void TableBuilder::WriteBlock(BlockBuilder* block, BlockHandle* handle) {
  ...
  switch (type) {
    ...
    case kZlibCompression: {
      ...
      if (compress((Bytef*)compressed->data(), &comp_len,
                   (const Bytef*)raw.data(), raw.size()) == Z_OK &&
          comp_len < raw.size() - (raw.size() / 8u)) {
        compressed->resize(comp_len);
        block_contents = *compressed;
      } else {
        block_contents = raw;
    ...
    case kZlibRawCompression: {
      ...
      if (deflateInit2(&strm, Z_DEFAULT_COMPRESSION, Z_DEFLATED, -15, 8,
        ...
        uLong bound = deflateBound(&strm, raw.size());
        ...
        if (deflate(&strm, Z_FINISH) == Z_STREAM_END &&
            strm.total_out < raw.size() - (raw.size() / 8u)) {
