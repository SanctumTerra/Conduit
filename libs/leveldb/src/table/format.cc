  ...
  switch (data[n]) {
    ...
    case kZlibCompression: {
      ...
      if (inflateInit(&strm) != Z_OK) {
        delete[] buf;
        return Status::Corruption("zlib inflateInit failed");
      }
      strm.next_in = (Bytef*)data;
      strm.avail_in = static_cast<uInt>(n);
      size_t out_cap = n * 4;
      char* ubuf = new char[out_cap];
      strm.next_out = (Bytef*)ubuf;
      strm.avail_out = static_cast<uInt>(out_cap);
      while (true) {
        int ret = inflate(&strm, Z_FINISH);
        if (ret == Z_STREAM_END) break;
        if (ret == Z_BUF_ERROR || (ret == Z_OK && strm.avail_out == 0)) {
          size_t have = out_cap - strm.avail_out;
          out_cap *= 2;
          char* nbuf = new char[out_cap];
          memcpy(nbuf, ubuf, have);
          delete[] ubuf;
          ubuf = nbuf;
          strm.next_out = (Bytef*)(ubuf + have);
          strm.avail_out = static_cast<uInt>(out_cap - have);
          continue;
        }
        inflateEnd(&strm);
        delete[] buf;
      ...
      size_t ulength = out_cap - strm.avail_out;
      inflateEnd(&strm);
      delete[] buf;
      result->data = Slice(ubuf, ulength);
      result->heap_allocated = true;
    ...
    case kZlibRawCompression: {
      ...
      if (inflateInit2(&strm, -15) != Z_OK) {
        delete[] buf;
        return Status::Corruption("zlib raw inflateInit2 failed");
      }
      strm.next_in = (Bytef*)data;
      strm.avail_in = static_cast<uInt>(n);
      size_t out_cap = n * 4;
      ...
      while (true) {
        ...
        delete[] buf;
        delete[] ubuf;
        return Status::Corruption("corrupted zlib raw compressed block contents");
      }
      size_t ulength = out_cap - strm.avail_out;
      inflateEnd(&strm);
      delete[] buf;
