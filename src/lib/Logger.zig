...
pub const Logger = struct {
    ...
    /// Caller must free the returned memory with freeTimeString().
    pub fn getTime() []const u8 {
        ...
        // Caller must free the returned memory using allocator.free()
        return std.fmt.allocPrint(allocator, "{d:0>2}:{d:0>2}:{d:0>2}.{d:0>3}", .{ h, m, s, ms }) catch |err| {
            std.debug.print("Error while printing time: {}\n", .{err});
            unreachable;
        };
    }
    ...
    /// Frees a string returned by getTime().
    pub fn freeTimeString(time_string: []const u8) void {
        const allocator = Callocator.get();
        allocator.free(time_string);
    }

    /// Returns a formatted date string (YYYY-MM-DD).
    /// Caller must free the returned memory with freeDateString().
    pub fn getDate() []const u8 {
        const allocator = Callocator.get();
        ...
        // Caller must free the returned memory using allocator.free()
        return std.fmt.allocPrint(allocator, "{d:0>4}-{d:0>2}-{d:0>2}", .{ unsigned_y, unsigned_mo, unsigned_da }) catch |err| {
            std.debug.print("Error while printing date: {}\n", .{err});
            unreachable;
        };
    }
    ...
    /// Frees a string returned by getDate().
    pub fn freeDateString(date_string: []const u8) void {
        const allocator = Callocator.get();
        allocator.free(date_string);
    }
};
