...
pub const Nack = struct {
    ...
    pub fn init(sequences: []const u32, allocator: std.mem.Allocator) !Nack {
    ...
    pub fn deserialize(data: []const u8) Nack {
        ...
        const result = Nack.init(sequences.items, CAllocator.get()) catch |err| {
            Logger.ERROR("Failed to initialize Nack: {}", .{err});
            sequences.deinit();
            return Nack{
                .sequences = &[_]u32{},
    ...
    pub fn serialize(self: *const Nack) []const u8 {
        const buffer = &[_]u8{};
        var stream = BinaryStream.init(buffer, 0);
        defer stream.deinit();
        // It fails with VarInt
        stream.writeUint8(Packets.Nack);
        ...
        return stream.toOwnedSlice() catch |err| {
            Logger.ERROR("Failed to serialize Nack: {}", .{err});
            return &[_]u8{};
        };
    }
};
