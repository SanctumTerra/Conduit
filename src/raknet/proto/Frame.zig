...
pub const CAllocator = @import("CAllocator");
...
pub const Frame = struct {
    ...
    pub fn init(reliable_frame_index: ?u32, sequence_frame_index: ?u32, ordered_frame_index: ?u32, order_channel: ?u8, reliability: Reliability, payload: []const u8, split_frame_index: ?u32, split_id: ?u16, split_size: ?u32, allocator: ?std.mem.Allocator) Frame {
        return .{ .reliable_frame_index = reliable_frame_index, .sequence_frame_index = sequence_frame_index, .ordered_frame_index = ordered_frame_index, .order_channel = order_channel, .reliability = reliability, .payload = payload, .split_frame_index = split_frame_index, .split_id = split_id, .split_size = split_size, .allocator = allocator };
    }
    ...
    pub fn read(stream: *BinaryStream) Frame {
        ...
        const owned_payload = CAllocator.get().dupe(u8, temp_payload) catch @panic("Failed to duplicate payload");
        return Frame.init(reliable_frame_index, sequence_frame_index, ordered_frame_index, order_channel, reliability, owned_payload, split_frame_index, split_id, split_size, CAllocator.get());
    }
