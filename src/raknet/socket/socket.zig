...
pub const CallbackFn = *const fn (data: []const u8, from_addr: std.net.Address, context: ?*anyopaque) void;
...
pub const Socket = struct {
    ...
    pub fn init(allocator: Allocator, host: []const u8, port: u16) SocketError!Self {
        ...
        // Validate the socket was created properly
        if (builtin.os.tag == .windows and self.socket_handle == std.os.windows.ws2_32.INVALID_SOCKET) {
            std.log.err("Socket initialization failed: invalid socket handle after creation", .{});
    ...
    pub fn listen(self: *Self) SocketError!void {
        ...
        // Ensure the socket is valid before binding
        if (builtin.os.tag == .windows and self.socket_handle == std.os.windows.ws2_32.INVALID_SOCKET) {
            std.log.err("Cannot listen: socket handle is invalid", .{});
            return SocketError.SocketCreationFailed;
        }
    ...
    fn bindSocket(self: *Self) SocketError!void {
        if (builtin.os.tag == .windows) {
            ...
            const result = std.os.windows.ws2_32.bind(self.socket_handle, @ptrCast(&sockaddr), @sizeOf(@TypeOf(sockaddr)));
            ...
            posix.bind(self.socket_handle, &self.bind_address.any, self.bind_address.getOsSockLen()) catch |err| {
    ...
    fn receiveLoop(self: *Self) void {
        ...
        while (!self.should_stop.load(.seq_cst)) {
            const result = self.receivePacket(&buffer) catch |err| {
                if (err == error.WouldBlock) {
                    std.time.sleep(std.time.ns_per_ms * 5);
                    continue;
                }
                std.log.err("Error receiving packet: {}", .{err});
                std.time.sleep(std.time.ns_per_ms * RECV_TIMEOUT_MS);
                continue;
            };

            if (result) |packet_info| {
                // Create a copy of the data since buffer is reused
                const data_copy = self.allocator.dupe(u8, packet_info.data) catch |err| {
                ...
                if (self.callback) |callback| {
                    callback(data_copy, packet_info.from_addr, self.context);
                } else {
                    // If there's no callback, we need to free the copy
    ...
    fn receivePacket(self: *Self, buffer: []u8) !?PacketInfo {
        if (builtin.os.tag == .windows) {
            var from_addr: std.net.Ip4Address = undefined;
            var addr_len: c_int = @sizeOf(@TypeOf(from_addr));

            const result = std.os.windows.ws2_32.recvfrom(self.socket_handle, buffer.ptr, @intCast(buffer.len), 0, @ptrCast(&from_addr), &addr_len);

            if (result == std.os.windows.ws2_32.SOCKET_ERROR) {
                const err = std.os.windows.ws2_32.WSAGetLastError();
                if (err == std.os.windows.ws2_32.WinsockError.WSAEWOULDBLOCK) {
            ...
            const bytes_received = posix.recvfrom(self.socket_handle, buffer, 0, &from_addr, &addr_len) catch |err| {
                if (err == error.WouldBlock) {
                    return error.WouldBlock;
                }
                return err;
    ...
    pub fn send(self: *Self, data: []const u8, to_addr: std.net.Address) SocketError!void {
        if (builtin.os.tag == .windows) {
            const result = std.os.windows.ws2_32.sendto(self.socket_handle, data.ptr, @intCast(data.len), 0, @ptrCast(&to_addr.in), @sizeOf(@TypeOf(to_addr.in)));
            ...
            posix.sendto(self.socket_handle, data, 0, &to_addr.any, to_addr.getOsSockLen()) catch |err| {
    ...
    pub fn getLocalAddress(self: *Self) SocketError!std.net.Address {
        if (builtin.os.tag == .windows) {
            ...
            if (std.os.windows.ws2_32.getsockname(self.socket_handle, @ptrCast(&addr), &addr_len) == std.os.windows.ws2_32.SOCKET_ERROR) {
                return SocketError.SocketCreationFailed;
            }
...
//     // Create a socket
//     var socket = Socket.init(allocator, "127.0.0.1", 8080) catch |err| {
//         std.log.err("Failed to create socket: {}", .{err});
//         return;
//     };
//     defer socket.deinit();
...
//     socket.setCallback(packetHandler, null);

//     // Start listening
//     socket.listen() catch |err| {
//         std.log.err("Failed to start listening: {}", .{err});
//         return;
//     };

//     std.log.info("Socket is listening. Press Ctrl+C to stop...");

//     // Send a test packet to ourselves
//     std.time.sleep(std.time.ns_per_s);
//     socket.sendTo("Hello, World!", "127.0.0.1", 8080) catch |err| {
//         std.log.err("Failed to send test packet: {}", .{err});
//     };
