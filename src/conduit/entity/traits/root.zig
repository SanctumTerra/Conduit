pub const EntityTrait = @import("./trait.zig").EntityTrait;
pub const EntityTraitInstance = @import("./trait.zig").EntityTraitInstance;
pub const EntityTraitVTable = @import("./trait.zig").EntityTraitVTable;
pub const EntityTraitConfig = @import("./trait.zig").EntityTraitConfig;
pub const Event = @import("./trait.zig").Event;
pub const initEntityTraitRegistry = @import("./trait.zig").initEntityTraitRegistry;
pub const deinitEntityTraitRegistry = @import("./trait.zig").deinitEntityTraitRegistry;
pub const applyGlobalTraits = @import("./trait.zig").applyGlobalTraits;
pub const inventory = @import("./inventory.zig");
pub const InventoryTrait = inventory.InventoryTrait;
pub const stats = @import("./stats.zig");
pub const StatsTrait = stats.StatsTrait;
pub const chunk_loading = @import("./chunk-loading.zig");
pub const ChunkLoadingTrait = chunk_loading.ChunkLoadingTrait;
pub const cursor = @import("./cursor.zig");
pub const CursorTrait = cursor.CursorTrait;
pub const gravity = @import("./gravity.zig");
pub const GravityTrait = gravity.GravityTrait;
pub const health = @import("./health.zig");
pub const HealthTrait = health.HealthTrait;
